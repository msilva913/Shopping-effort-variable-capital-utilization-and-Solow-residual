...
Gam = C-ha*C(-1) - exp(theta_D)*D^(1+1/eta)/(1+1/eta) - theta_N_ss*exp(theta_N)*N^(1+1/nu)/(1+1/nu);
...
steady_state_model;
    ...
    theta_N_s = (1-phi)*W/(N^(1/nu)*mu_ss);
    Gam = (C*(1-ha) - D^(1+1/eta)/(1+1/eta) - theta_N_s*N^(1+1/nu)/(1+1/nu));
    ...
    R_mc = W*exp(g)*(alpha_K_ss/alpha_N_ss)*N_mc/K_mc;
    R_sc = W*exp(g)*(alpha_K_ss/alpha_N_ss)*N_sc/K_sc;
    R_I = R_mc;
...
mode_file=BRS_sectoral_perfect_mobility_mh_mode, %With _mh option uses mode after MCM run
//nograph,
load_mh_file, 
//mh_recover,
