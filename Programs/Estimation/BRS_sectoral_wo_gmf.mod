...
#Z_mc_ss = (Y_mc_ss + nu_mc)/(exp(g_bar)^(-alpha_K)*K_mc_ss^(alpha_K)*N_mc_ss^(alpha_N));
#Z_sc_ss = (Y_sc_ss + nu_sc)/(exp(g_bar)^(-alpha_K)*K_sc_ss^(alpha_K)*N_sc_ss^(alpha_N));
#Z_I_ss = (I_ss + nu_I)/(exp(g_bar)^(-alpha_K)*K_I_ss^(alpha_K)*N_I_ss^(alpha_N));

#zeta_ss = C_ss*(1-ha);
#theta_N_ss = W_ss/(N_ss^(1/nu)*zeta_ss*mu_ss);

[name = 'Labor composite']
N_comp = (omega^(-theta)*N_C^(1+theta) + (1-omega)^(-theta)*N_I^(1+theta))^(1/(1+theta));

[name='Labor leisure:C']
theta_N_ss*exp(theta_N)*(N_comp)^(1/nu)*(N_C/N_comp)^theta*omega^(-theta) = W_C/(mu_ss*exp(mu_C)*zeta);

[name='Labor leisure:I']
theta_N_ss*exp(theta_N)*(N_comp)^(1/nu)*(N_I/N_comp)^theta*(1-omega)^(-theta)  = W_I/(mu_ss*exp(mu_I)*zeta);
...
Gam = C-ha*C(-1) -  theta_N_ss*exp(theta_N)*N_comp^(1+1/nu)/(1+1/nu)*zeta;

[name = 'Law of motion of wealth effects variable']
zeta =  (C-ha*C(-1))^(gam)*zeta(-1)^(1-gam);
...
Y_mc = (Z_mc_ss*exp(g)^(-alpha_K)*exp(Z_C)*(h_mc*K_mc(-1))^alpha_K*(N_mc)^alpha_N-nu_mc);

[name = 'Services production']
Y_sc = (Z_sc_ss*exp(g)^(-alpha_K)*exp(Z_C)*(h_sc*K_sc(-1))^alpha_K*(N_sc)^alpha_N-nu_sc);

[name = 'Investment production']
I = (Z_I_ss*exp(g)^(-alpha_K)*exp(Z_I)*(h_I*K_I(-1))^alpha_K*(N_I)^alpha_N-nu_I);
...
W_C/p_mc = alpha_N*(Y_mc+nu_mc)/N_mc;

[name = 'Labor demand:sc']
W_C/p_sc = alpha_N*(Y_sc+nu_sc)/N_sc;

[name = 'Labor demand:I']
W_I/p_I = alpha_N*(I+nu_I)/N_I;

[name = 'Capital demand:mc']
R_mc/p_mc = exp(g)*alpha_K*(Y_mc+nu_mc)/(h_mc*K_mc(-1));

[name = 'Capital demand:sc']
R_sc/p_sc = exp(g)*alpha_K*(Y_sc+nu_sc)/(h_sc*K_sc(-1));

[name = 'Capital demand:I']
R_I/p_I = exp(g)*alpha_K*(I+nu_I)/(h_I*K_I(-1));
...
util_ND = (Z_mc_ss*exp(g)^(-alpha_K)*exp(Z_C)*(h_mc*K_mc(-1))^alpha_K*N_mc^alpha_N-nu_mc)/(Z_mc_ss*exp(g)^(-alpha_K)*exp(Z_C)*K_mc(-1)^alpha_K*N_mc^alpha_N-nu_mc);

util_D = (Z_I_ss*exp(g)^(-alpha_K)*exp(Z_I)*(h_I*K_I(-1))^alpha_K*N_I^alpha_N-nu_I)/(Z_I_ss*exp(g)^(-alpha_K)*exp(Z_I)*K_I(-1)^alpha_K*N_I^alpha_N-nu_I);
