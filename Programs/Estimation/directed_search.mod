...
Y_obs, TI_obs, p_I_obs, lab_prod_obs;
...
var_share, Gamma_bar, phi_I, Psi, eta, phi, Psi_K,
% persistence parameters
rho_Z, rho_ZI , rho_shop, rho_zeta, rho_b;
...
(epsi-1)*(p_C-P_H) = phi*S_H + (1-phi)*N_H;
...
//YC_Y*(YC) + I_Y*(I) = d_Y*d + omega*(w+L) + (1-d_Y-omega)*(phi_C*(r_C+K_C) + phi_I*(r_I+K_I));


% Technology shocks: common and investment-specific

Z = rho_Z*Z(-1) + e_Z;
u_I = rho_ZI*Z_I(-1) + e_ZI;
Z_I = Z + u_I;
...
SR_obs = Y_obs - (1-omega)*K - omega*L;
SR_util_obs = Y_util_obs - (1-omega)*K - omega*L;
...
varobs Y_obs, TI_obs, lab_prod_obs, L_obs;
...
Psi, 0.25, 0.0, 0.5,          BETA_PDF, 0.25, 0.25, 0, 0.5;

% Gross markup
Gamma_bar, 1.3, 1.01, 2.0,    BETA_PDF, 1.3, 0.5, 1.01, 2.0;

% Elasticity parameter matching function
gam, 1.0, 0.01, 2.0,          BETA_PDF, 1.0, 0.6, 0.0, 2.0;
...
stderr e_b, 0.027, 0.001, 0.4, INV_GAMMA_PDF, 0.01, 0.004;
...
C, C_obs, S_H, Y_obs, TI_obs, SR_obs, SR_util_obs, p_I_obs, L;
