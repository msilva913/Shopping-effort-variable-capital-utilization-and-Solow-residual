...
Y_obs, C_obs, TI_obs, LC_obs, LI_obs, p_I_obs, lab_prod_obs, labor_share, SR_obs;
...
% 19)
[name =  'C production']
C = (p_C + Z + alpha*(u_C+K_C(-1))+(1-alpha)*L_C);

% 20) 
[name = 'I production']
I = (p_I + Z_I + alpha*(u_I+K_I(-1)) + (1-alpha)*L_I);
