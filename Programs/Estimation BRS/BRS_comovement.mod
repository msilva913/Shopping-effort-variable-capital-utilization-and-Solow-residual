...
LC_obs = log_NC - steady_state(log_NC);
LI_obs = log_NI - steady_state(log_NI);
...
eta, 0.20, 0.00, 2.0,          GAMMA_PDF, 0.2, 0.5;

% Persistence parameters
rho_Z,  0.9, 0.01, 0.99999,        BETA_PDF, 0.6, 0.2;
rho_ZI,  0.9, 0.01, 0.999999,        BETA_PDF, 0.6, 0.2;
rho_N,  0.9, 0.01, 0.9999,        BETA_PDF, 0.6, 0.2;
...
stderr e_N, 0.01, 0.00001, 0.2,  INV_GAMMA_PDF, 0.01, 0.1;
stderr e_D, 0.01, 0.00001, 0.5,  INV_GAMMA_PDF, 0.01, 0.1;

end;

options_.TeX=1;

varobs I_obs, C_obs, LC_obs, LI_obs;

estimation(optim=('MaxIter', 200), 
datafile=observables, 
mode_file=BRS_comovement_mode, 
//load_mh_file, 
//mh_recover,
...
prior_trunc=0,
tex)
I_obs, C_obs, Y_obs, LC_obs, LI_obs, lab_prod_obs, p_I_obs, log_D;
