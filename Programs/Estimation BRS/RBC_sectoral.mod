...
zeta =  (C-ha*C(-1))^(sigma_s)*zeta(-1)^(1-sigma_s);
...
C = (Z_C_ss*exp(g)^(-alpha_K)*exp(Z_C)*(h_C*K_C(-1))^alpha_K*(N_C)^alpha_N-nu_C);
...
[name = 'Labor demand:C']
W_C = alpha_N*(C+nu_C)/N_C;

[name = 'Labor demand:I']
W_I/p_I = alpha_N*(I+nu_I)/N_I;

[name = 'Capital demand:C']
R_C = exp(g)*alpha_K*(C+nu_C)/(h_C*K_C(-1));
...
util_ND = ((h_C*K_C(-1))^alpha_K*N_C^alpha_N-nu_C)/(K_C(-1)^alpha_K*N_C^alpha_N-nu_C);

util_D = ((h_I*K_I(-1))^alpha_K*N_I^alpha_N-nu_I)/(K_I(-1)^alpha_K*N_I^alpha_N-nu_I);
...
steady_state_model;
    ...
    util_ND = ((h_C*K_C)^alpha_K_ss*N_C^alpha_N_ss-nu_C_ss)/(K_C^alpha_K_ss*N_C^alpha_N_ss-nu_C_ss);
    util_D = ((h_I*K_I)^alpha_K_ss*N_I^alpha_N_ss-nu_I_ss)/(K_I^alpha_K_ss*N_I^alpha_N_ss-nu_I_ss);
    util = (C/Y)*util_ND + (I/Y)*util_D;
