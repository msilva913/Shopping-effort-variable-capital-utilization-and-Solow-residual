...
#W_ss = p_I_ss*alpha_N*I_ss/N_I_ss;

#Z_C_ss = (1-I_Y)/(exp(g_bar)^(-alpha_K)*K_C_ss^(alpha_K)*N_C_ss^(alpha_N));
#Z_I_ss = (I_Y)/(exp(g_bar)^(-alpha_K)*K_I_ss^(alpha_K)*N_I_ss^(alpha_N));

#theta_N_ss = W_ss/(N_ss^(1/nu));


[name='Labor leisure']
theta_N_ss*exp(theta_N)*N^(1/nu) = W;

[name = 'Composite utility term']
Gam = (C - theta_N_ss*exp(theta_N)*N^(1+1/nu)/(1+1/nu));

[name= 'Euler equation: C']
Gam^(-sigma)*p_I = beta*(R_C(+1) + p_I(+1)*(1-delta))*Gam(+1)^(-sigma)*exp(g(+1))^(-sigma);

[name = 'Euler equation: I']
R_I(+1) = R_C(+1);

[name = 'Consumption production']
C = Z_C_ss*exp(Z_C)*exp(g)^(-alpha_K)*(K_C(-1))^alpha_K*(N_C)^alpha_N;

[name = 'Investment production']
I = Z_I_ss*exp(Z_I)*exp(g)^(-alpha_K)*(K_I(-1))^alpha_K*(N_I)^alpha_N;
...
R_C = alpha_K*C/(K_C(-1))*exp(g);

[name = 'Labor demand:I']
W/p_I = alpha_N*I/N_I;

[name = 'Capital demand:I']
R_I/p_I = alpha_K*I/(K_I(-1))*exp(g);
...
steady_state_model;
    ...
    alpha_N_ss = labor_share;
    W = alpha_N_ss*I/N_I*p_I;
    theta_N_s = W/(N^(1/nu));
    Gam = C - theta_N_s*N^(1+1/nu)/(1+1/nu);
...
varobs I_obs, Y_obs, p_I_obs, Y_N_obs, util_obs;


estimation(optim=('MaxIter', 200), 
datafile=observables_sectoral, 
mode_file=BRS_util_mode, 
load_mh_file, 
//mh_recover,
...
Y_obs, Y_N_obs, SR_obs, I_obs, p_I_obs, C_obs, NC_obs, NI_obs;
