...
#theta_N_ss = (1-phi)*W_ss*Gam_ss^(-gam)/(N_ss^(1/nu));


[name='Labor leisure']
theta_N_ss*exp(theta_N)*N^(1/nu) = (1-phi)*W*Gam^(-gam);
...
Gam = (C - exp(theta_D)*D^(1+1/eta)/(1+1/eta)) ;
...
steady_state_model;
    ...
    theta_N_s = (1-phi_ss)*W*Gam^(-gam)/(N^(1/nu));
