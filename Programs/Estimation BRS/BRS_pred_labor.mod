...
C = A_C*(D_C)^phi*(Z_C_ss*exp(g)^(-alpha_K)*exp(Z_C)*(h_C*K_C(-1))^alpha_K*(N_C(-1))^alpha_N-nu_C);

[name = 'Investment production']
I = A_I*(D_I)^phi*(Z_I_ss*exp(g)^(-alpha_K)*exp(Z_I)*(h_I*K_I(-1))^alpha_K*(N_I(-1))^alpha_N-nu_I);
...
[name = 'Labor demand:C']
(1-phi)*W_C = alpha_N*(C+A_C*D_C^phi*nu_C)/N_C(-1);

[name = 'Labor demand:I']
(1-phi)*W_I/p_I = alpha_N*(I+A_I*D_I^phi*nu_I)/N_I(-1);
...
util = (C/Y)*A_C*D_C^phi*((h_C*K_C(-1))^alpha_K*N_C(-1)^alpha_N-nu_C)/(K_C(-1)^alpha_K*N_C(-1)^alpha_N-nu_C) + 
(I/Y)*A_I*D_I^phi*((h_I*K_I(-1))^alpha_K*N_I(-1)^alpha_N-nu_I)/(K_I(-1)^alpha_K*N_I(-1)^alpha_N-nu_I);
