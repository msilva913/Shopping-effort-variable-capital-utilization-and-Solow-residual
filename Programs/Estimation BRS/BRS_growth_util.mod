...
util = (C/Y)*A_C*D_C^(phi) + (I/Y)*A_I*D_I^(phi);
...
steady_state_model;
    ...
    util = (C/Y)*A_C_ss*D_C^(phi_ss) + (I/Y)*A_I_ss*D_I^(phi_ss);
...
Y_obs, Y_N_obs, I_obs, p_I_obs, C_obs, util_obs, D_obs,
log_Y, log_Y_N, log_I, log_p_I, log_C, log_util, log_D;
