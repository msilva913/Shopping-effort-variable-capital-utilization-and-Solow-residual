...
parameters A, beta, sigma_a, psi_inv, sigma, 
...
%1) Map Gamma and Psi back to rho and phi
#Psi_inv = 1/Psi;
#rho = (sqrt((2+Gamma_bar-Psi_inv)^2+4*(Psi_inv-1))+(2+Gamma_bar-Psi_inv))/2;
#phi = 1-Gamma_bar/rho;

%2) Consistency of alpha with labor share
#alpha = 1-Gamma_bar*wL_Y;
#alpha_2 = var_share*(1-alpha);

%3) Consistency of delta_K with investment share
#delta_K = r*(alpha/(phi_I*Gamma_bar) - 1)^(-1);
...
Q_I = lam(+1)-lam + (r+delta_K)/(1+r)*(I(+1)-K_I(+1)-u_I(+1)) + Q_I(+1)/(1+r);
...
I =  Z_I + alpha*(u_I+K_I) + (1-alpha)*L_I;
...
I_util = Z_I + alpha*(K_I) + (1-alpha)*(L_I);
...
cu = phi_C*(phi*q_C + alpha*u_C) + phi_I*(alpha*u_I);
...
TI_obs = I;
Y_obs = phi_C*(C-p_C) + phi_I*(I);
Y_util_obs = phi_C*(C_util-p_C) + phi_I*(I_util);
...
C_obs, Y_obs, D, lab_prod_obs, SR_obs, cu, TI_obs, L_C, L_I, L, p_I_obs, Gamma_C ;
