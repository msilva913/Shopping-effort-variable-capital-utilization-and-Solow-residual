...
#Gam_ss = C_ss - D_ss^(1+1/eta)/(1+1/eta);

#theta_N_ss = (1-phi)*Gam_ss^(-gam)*W_ss/(N_comp^(1/nu));
...
theta_N_ss*exp(theta_N)*(N_comp)^(1/nu)*(N_C/N_comp)^theta*omega^(-theta) = exp(theta_C)*Gam^(-gam)*(1-phi)*W_C;

[name='Labor leisure:I']
theta_N_ss*exp(theta_N)*(N_comp)^(1/nu)*(N_I/N_comp)^theta*(1-omega)^(-theta)  = exp(theta_C)*Gam^(-gam)*(1-phi)*W_I;
...
Gam = exp(theta_C)*C - exp(theta_D)*D^(1+1/eta)/(1+1/eta);
...
steady_state_model;
    ...
    W_C = labor_share*Y/N;
    W_I = W_C;
    Gam = (C - D^(1+1/eta)/(1+1/eta));
    theta_N_s = (1-phi_ss)*W_C*Gam^(-gam)/(N_comp^(1/nu));
