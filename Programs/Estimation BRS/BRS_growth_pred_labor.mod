...
var Y           ${Y}$ (long_name='output')
    ...
    theta_C     ${\theta_C}$ (long_name='Consumption preferene shock')
    ...
    ha         ${ha}$ (long_name = 'External habit stock')
...
[name='Labor leisure:C']% change with predetermined labor
...
Gam = (exp(theta_C)*C - exp(theta_D)*D^(1+1/eta)/(1+1/eta) - theta_N_ss*exp(theta_N)*N(-1)^(1+1/nu)/(1+1/nu) - zeta*ha);

[name = 'External habit stock']
ha = exp(theta_C)*C(-1) - exp(theta_D)*D(-1)^(1+1/eta)/(1+1/eta) - theta_N_ss*exp(theta_N)*N(-2)^(1+1/nu)/(1+1/nu);
...
C = A_C*(D_C)^phi*Z_C_ss*exp(g)^(-alpha_K)*(h_C*K_C(-1))^alpha_K*(N_C(-1))^alpha_N;

[name = 'Investment production']
I = A_I*(D_I)^phi*Z_I_ss*exp(Z_I)*exp(g)^(-alpha_K)*(h_I*K_I(-1))^alpha_K*(N_I(-1))^alpha_N;
...
[name = 'Labor demand:C']
(1-phi)*W_C = alpha_N*C/N_C(-1);

[name = 'Labor demand:I']
(1-phi)*W_I/p_I = alpha_N*I/N_I(-1);
...
(1-phi)*R_I = alpha_K*I/(h_C*K_I(-1))*exp(g);
...
steady_state_model;
    ...
    W_C = alpha_N_ss*I/N_I*p_I/(1-phi);
    W_I = W_C;
    theta_N_s = (1-phi)*W/(N^(1/nu));
    Gam = (C - D^(1+1/eta)/(1+1/eta) - theta_N_s*N^(1+1/nu)/(1+1/nu))*(1-zeta);
    ha = (C - D^(1+1/eta)/(1+1/eta) - theta_N_s*N^(1+1/nu)/(1+1/nu));
...
mode_file=BRS_growth_ext_util_mode, 
//load_mh_file, 
//mh_recover,
