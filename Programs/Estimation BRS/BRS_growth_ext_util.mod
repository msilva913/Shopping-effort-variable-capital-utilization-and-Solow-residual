...
Gam = (exp(theta_C)*C - exp(theta_D)*D^(1+1/eta)/(1+1/eta) - theta_N_ss*exp(theta_N)*N^(1+1/nu)/(1+1/nu) - zeta*ha);

[name = 'External habit stock']
ha = exp(theta_C)*C(-1) - exp(theta_D)*D(-1)^(1+1/eta)/(1+1/eta) - theta_N_ss*exp(theta_N)*N(-1)^(1+1/nu)/(1+1/nu);
