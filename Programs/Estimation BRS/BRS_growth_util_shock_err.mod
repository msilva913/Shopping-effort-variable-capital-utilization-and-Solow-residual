...
var Y           ${Y}$ (long_name='output')
    ...
    theta_h     ${\theta_h}$ (long_name='Capacity utilization depreciation shock')
...
varexo e_g ${e_g}$ (long_name= 'Labor-augmenting-technology growth shock')
       ...
       e_h ${e_h}$ (long_name='Capacity utilization depreciation shock')
    ;
...
parameters 
    ...
    rho_h   ${\rho_h}$ (long_name = 'persistence utilization depreciation shock')
...
delta_C = exp(theta_h)*(delta + sigma_b*(h_C-1) + sigma_a*sigma_b/2*(h_C-1)^2);

[name = 'Depreciation rate: I']
delta_I = exp(theta_h)*(delta + sigma_b*(h_I-1) + sigma_a*sigma_b/2*(h_I-1)^2);

[name = 'Depreciation rate derivative: C']
delta_C_pr = exp(theta_h)*(sigma_b + sigma_a*sigma_b*(h_C-1));

[name = 'Depreciation rate derivative: I']
delta_I_pr = exp(theta_h)*(sigma_b + sigma_a*sigma_b*(h_I-1));
...
[name =' Cost of varying capital utilization process']
theta_h = rho_h*theta_h(-1) - e_h;
...
rho_h, 0.6, 0.01, 0.9999,        BETA_PDF, 0.6, 0.2;
...
stderr e_h, 0.01, 0.0001, 0.4,  INV_GAMMA_PDF, 0.01, 0.1;
...
//moments_varendo,
prior_trunc=0)
Y_obs, Y_N_obs, I_obs, p_I_obs, C_obs, N_obs, NC_obs, NI_obs, util_obs;
...
Y_obs, Y_N_obs, I_obs, p_I_obs, C_obs, N_obs, NC_obs, NI_obs, util_obs, D_obs,
