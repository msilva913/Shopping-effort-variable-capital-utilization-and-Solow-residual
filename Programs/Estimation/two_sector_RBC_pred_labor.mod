...
parameters A, beta, delta_K, sigma_a, psi_inv, sigma, 
 Gamma_bar, wL_Y, phi_I, Psi_K, iota
...
C = (1+nu_R)*( p_C + Z + alpha*(u_C+K_C(-1))+(1-alpha)*L_C)-nu_R*(p_C);

% 20) 
[name = 'I production']
I = (1+nu_R)*(p_I + Z_I + alpha*(u_I+K_I(-1)) + (1-alpha)*L_I) - nu_R*(p_I);
...
//cu = phi_C*(alpha*u_C) + phi_I*(alpha*u_I);
...
iota, 0.01, 0.0, 0.99,         BETA_PDF, 0.5, 0.25;
