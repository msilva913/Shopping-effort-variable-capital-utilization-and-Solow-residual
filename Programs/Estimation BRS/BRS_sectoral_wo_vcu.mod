...
Q_mc = beta*exp(theta_b)*(lam(+1)/lam)*exp(g(+1))^(-sigma)*(R_mc(+1) + (1-delta)*Q_mc(+1));

[name= 'Euler equation: sc']
Q_sc = beta*exp(theta_b)*(lam(+1)/lam)*exp(g(+1))^(-sigma)*(R_sc(+1) + (1-delta)*Q_sc(+1));

[name = 'Euler equation: I']
Q_I = beta*exp(theta_b)*(lam(+1)/lam)*exp(g(+1))^(-sigma)*(R_I(+1) + (1-delta)*Q_I(+1));

[name = 'Non-durable goods production']
Y_mc = A_mc*(D_mc)^phi*(Z_mc_ss*exp(g)^(-alpha_K)*exp(Z_C)*(K_mc(-1))^alpha_K*(N_mc)^alpha_N-nu_mc);

[name = 'Services production']
Y_sc = A_sc*(D_sc)^phi*(Z_sc_ss*exp(g)^(-alpha_K)*exp(Z_C)*(K_sc(-1))^alpha_K*(N_sc)^alpha_N-nu_sc);
...
K_mc*exp(g) = (1-Smc)*I_mc*exp(g) + (1-delta)*K_mc(-1);

[name = 'Capital law of motion:sc']
K_sc*exp(g) = (1-Ssc)*I_sc*exp(g) + (1-delta)*K_sc(-1);

[name = 'Capital law of motion:I']
K_I*exp(g) = (1-Si)*I_I*exp(g) + (1-delta)*K_I(-1);
...
(1-phi)*R_mc/p_mc = exp(g)*alpha_K*(Y_mc+A_mc*D_mc^phi*nu_mc)/(K_mc(-1));

[name = 'Capital demand:sc']
(1-phi)*R_sc/p_sc = exp(g)*alpha_K*(Y_sc+A_sc*D_sc^phi*nu_sc)/(K_sc(-1));
...
mode_file=BRS_sectoral_wo_vcu_mh_mode, %With _mh option uses mode after MCM run
//nograph,
load_mh_file, 
//mh_recover,
...
//moments_varendo,
prior_trunc=0)
Y_obs, Y_N_obs, I_obs, p_I_obs, C_obs, NC_obs, NI_obs, util_ND_obs, util_D_obs, SR_obs, util_obs, D_obs;
...
Y_obs, Y_N_obs, SR_obs, I_obs, p_I_obs, C_obs, NC_obs, NI_obs, util_ND_obs, util_D_obs, util_obs, D_obs, tech_obs;
//log_Y, log_Y_N, log_SR, log_I, log_p_I, log_C, log_N, log_NC, log_NI, log_util_ND, log_util_D, log_util, log_D, log_tech;
